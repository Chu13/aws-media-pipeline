...
Slot into the array after Level 05 (Holograma). The tool now has a real,
independent brand — **Ember** — designed specifically for this project (see
`docs/brand/brand-guidelines.md` and `docs/brand/logo.svg` in this repo if
you want the full rationale, palette, and voice behind it).

```ts
{
  slug: "aws-media-pipeline",
  levelNumber: "06",
  title: "Ember — Serverless Image Optimizer",
  ...
  coverImageAlt:
    "The Ember tool after a completed run: the glowing-ember logo and tagline 'Smaller images. Shorter memory.' above a status line reading 'Done — six variants, up to 99.8% smaller. See for yourself below,' with the How It Works section's four steps visible beneath it.",
  problem:
...
screenshot of the live, rebranded tool mid-result (not a mockup). Copy it
into Chu-Website as:
...
The `coverImageAlt` text above already describes what's visible in it.

## 2a. Open Graph image (bonus, not part of the standard contract)

The live tool now serves its own OG card at
`https://d1xrhd1t6kp6bq.cloudfront.net/og-image.png` (1200×630) with real
`og:*`/`twitter:*` meta tags on the demo page itself — so sharing the
`demoUrl` directly (outside the Chu-Website embed) already renders a proper
social card. No action needed here; just flagging that it exists.
...
```ts
{
  slug: "ember-is-live-level-06",
  date: "2026-08-02",
  title: "Ember is live — Level 06",
  description:
